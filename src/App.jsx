...
import mapData from './maps/1500.json';
...
export default function TarihselHarita() {
  ...
  const [activeStyle,  setActiveStyle]  = useState('topo_v2');
  ...
  const injectLayers = useCallback((map) => {
    ...
    map.addLayer({
      ...
      paint: {
        'line-color':   ['coalesce', ['get', 'strokeColor'], '#2c5f8a'],
        'line-width':   [
          'interpolate', ['linear'], ['zoom'],
          3, ['coalesce', ['get', 'strokeWidth'], 1],
          9, ['*', ['coalesce', ['get', 'strokeWidth'], 1], 2],
        ],
        'line-opacity': 0.85,
      },
    }, before);
